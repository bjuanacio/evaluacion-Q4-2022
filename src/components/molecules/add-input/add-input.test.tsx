...
describe("AddInput tests", () => {
  ...
  it("should render one input and one button", () => {
    render(<AddInput onAdd={() => {}} placeHolder="Este es un placeholer" />);

    const button = screen.getByText("Agregar");
    expect(button).toBeInTheDocument();

    const input = screen.getByPlaceholderText("Este es un placeholer");
  ...
  it("should execute onChangeText and execute onAdd", async () => {
    ...
    render(
      <AddInput onAdd={mockOnAdd} placeHolder="Este es un placeholer" />
    );
